    ...
    // ── InventoryCombobox ──────────────────────────────────────────────────────────
    function InventoryCombobox({ value, onChange, inventoryList, placeholder, label, currentProductId, products }) {
      const [open, setOpen] = useState(false);
    ...
    function EditProductModal({ product, inventoryList, onClose, onSave, onPriceError, products }) {
        ...
        <div className="modal-overlay" style={{ zIndex: 3000 }}>
          <div className="modal-content modal-content-sm" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-actions">
              ...
              <button type="button" className={confirmClass || 'btn-primary'} onClick={onConfirm} disabled={confirmLabel === 'Processing...'}
                style={{ opacity: confirmLabel === 'Processing...' ? 0.6 : 1, cursor: confirmLabel === 'Processing...' ? 'not-allowed' : 'pointer' }}>
                {confirmLabel}
              </button>
            </div>
          </div>
        ...
        <div className="modal-overlay" onClick={onClose} style={{ zIndex: 4000 }}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-actions">
              ...
              <button type="button" className="btn-primary" onClick={onConfirm} disabled={isSubmitting} style={{ opacity: isSubmitting ? 0.6 : 1 }}>
    ...
    // ── Price Error Modal ──────────────────────────────────────────────────────────
    function PriceErrorModal({ isOpen, onClose, message, title = 'Price Required' }) {
    ...
    export default function ProductListPage() {
      ...
      const handleConfirmEditSave = async () => {
        ...
        try {
            ...
            const updatedProduct = await updateProduct(productId, productDataForApi, token);
            ...
            setPriceErrorMessage(error.message || 'Failed to save product. Please try again.');
      ...
      const executeDelete = async () => {
        ...
        try {
          ...
          setPriceErrorMessage(error.message || 'Failed to delete product. Please try again.');
      ...
      const executeArchive = async (product) => {
        ...
        try {
          ...
          setPriceErrorMessage(error.message || 'Failed to archive product. Please try again.');
      ...
      const handleRestore = async (product) => {
        ...
        try {
          ...
          setPriceErrorMessage(error.message || 'Failed to restore product. Please try again.');
      ...
      const executeBulkAction = async () => {
        ...
        try {
          ...
          setPriceErrorMessage(error.message || 'Failed to perform this action. Please try again.');
        ...
        <ErrorBoundary>
              ...
              <table className="inventory-table" style={{ 
                ...
                <tbody>
                  {filteredProducts.map(product => {
                    ...
                    return (
                      <React.Fragment key={(product._id?.toString?.() ?? product._id) || product.id}>
          ...
          {showPriceErrorModal && (
            <PriceErrorModal
              ...
              onClose={() => { setShowPriceErrorModal(false); setPriceErrorTitle('Price Required'); }}
