...
// ── Integer Input ──────────────────────────────────────────────────────────────
export function IntegerInput({ value, onChange, min = 0, max, placeholder, style, disabled }) {
  return (
    <input
...
// ── Decimal Input ──────────────────────────────────────────────────────────────
export function DecimalInput({ value, onChange, placeholder, style, disabled }) {
  return (
    <input
...
// ── Issue Type Config (for Goods Issue tab) ────────────────────────────────────
...
export function deductFromBatchesFIFO(batches, qtyToDeduct) {
  ...
  return { success: remaining === 0, deducted, remaining, error: remaining > 0 ? `Insufficient stock. Could only deduct ${deducted} of ${qtyToDeduct}.` : null };
}
...
export function restoreStockToBatch(material, poNumber, qtyToRestore) {
  if (!material || !material.batches || !Array.isArray(material.batches)) return false;

  const updatedBatches = material.batches.map(batch => {
    if (batch.poNumber === poNumber && (batch.qtyDamaged || batch.damagedQty || 0) > 0) {
