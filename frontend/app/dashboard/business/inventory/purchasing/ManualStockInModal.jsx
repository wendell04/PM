  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      <div onClick={() => setOpen(o => !o)}
        style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', padding: '0.65rem 0.875rem', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', color: '#E5E2E1', fontSize: '0.85rem', cursor: 'pointer', userSelect: 'none' }}>
        <span>{display}</span>
        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" style={{ flexShrink: 0, color: 'var(--gray)', transform: open ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s' }}>
          <path d="M6 9l6 6 6-6"/>
        </svg>
      </div>

      {open && (
        <div style={{ position: 'absolute', top: 'calc(100% + 4px)', left: 0, right: 0, background: '#1A1A1A', border: '1px solid rgba(255,255,255,0.12)', borderRadius: '8px', zIndex: 50, overflow: 'hidden', boxShadow: '0 8px 32px rgba(0,0,0,0.5)', maxHeight: '260px', overflowY: 'auto' }}>

          {/* General option */}
          <button type="button"
            onClick={() => { onChange('', 'General Merchandise'); setOpen(false); }}
            style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.65rem 0.875rem', background: !value ? 'rgba(212,168,67,0.12)' : 'transparent', border: 'none', cursor: 'pointer', textAlign: 'left', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
            <div>
              <div style={{ fontSize: '0.82rem', fontWeight: 600, color: !value ? '#D4A843' : '#E5E2E1' }}>General Merchandise</div>
              <div style={{ fontSize: '0.65rem', color: 'var(--gray)', marginTop: '0.1rem' }}>Local market / various vendors</div>
            </div>
            {!value && <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#D4A843" strokeWidth="3"><polyline points="20 6 9 17 4 12"/></svg>}
          </button>
          ...
          {eligibleVendors.length === 0 && selectedCategories.length > 0 && (
            <div style={{ padding: '1rem 0.875rem', textAlign: 'center', color: 'var(--gray)', fontSize: '0.75rem' }}>
          ...
          <button type="button" onClick={() => { setOpen(false); onAddNew(); }}
            style={{ width: '100%', display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.65rem 0.875rem', background: 'transparent', border: 'none', borderTop: '1px solid rgba(255,255,255,0.08)', cursor: 'pointer', color: '#D4A843', fontSize: '0.82rem', fontWeight: 600 }}>
...
function AddVendorQuickModal({ isOpen, onClose, onAdd, existingVendors, selectedCategories }) {
  ...
  const handleSubmit = () => {
    if (!form.name.trim()) { setInfoModal({ title: 'Validation Error', message: 'Please enter a vendor name.' }); return; }
    ...
    if (existingVendors.some(v => v.name.toLowerCase() === form.name.trim().toLowerCase())) {
      setInfoModal({ title: 'Duplicate Vendor', message: `"${form.name.trim()}" already exists.` }); return;
    }
      ...
      <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: '520px', width: '90%' }}>
        ...
        <div className="modal-body">
          ...
          {form.categories.length > 0 && (
            <div style={{ padding: '0.65rem 0.875rem', background: 'rgba(212,168,67,0.1)', border: '1px solid rgba(212,168,67,0.3)', borderRadius: '8px', marginBottom: '1rem', fontSize: '0.72rem', color: '#D4A843' }}>
              <strong>Auto-linked categories:</strong> {form.categories.join(', ')} — this vendor will be eligible for future stock-ins of these items.
            </div>
          )}
          ...
          <div className="form-group">
            ...
            <input type="email" className="form-input" value={form.email}
              ...
              placeholder="e.g., vendor@email.com" maxLength={100} />
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
            <div className="form-group">
              <label className="form-label">Phone</label>
              <input type="text" className="form-input" value={form.phone} onChange={handlePhoneChange}
        ...
        <div className="modal-actions">
          ...
          <button type="button" className="btn-primary" onClick={handleSubmit}>Add Vendor</button>
        </div>
      </div>
      {infoModal && (
        <div className="modal-overlay" style={{ zIndex: 1200 }}>
          <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: '400px' }}>
            <div className="modal-header"><h2 className="modal-title">{infoModal.title}</h2></div>
            <div style={{ padding: '1.5rem 2rem', fontSize: '0.875rem', color: '#E5E2E1' }}>{infoModal.message}</div>
            <div className="modal-actions"><button className="btn-primary" onClick={() => setInfoModal(null)}>OK</button></div>
          </div>
        </div>
      )}
    </div>
...
// ── Integer Input ──────────────────────────────────────────────────────────────
function IntegerInput({ value, onChange, min = 0, max, placeholder, style, disabled, qtyValue }) {
  ...
  return (
    <input type="text" value={value} inputMode="numeric" pattern="[0-9]*"
...
function DecimalInput({ value, onChange, placeholder, style, disabled, max = 9999999.99 }) {
  ...
  const handleWheel = (e) => { if (document.activeElement === e.target) e.target.blur(); };
  return (
    <input type="text" value={value} inputMode="decimal"
      placeholder={placeholder} disabled={disabled} style={style}
      onChange={handleChange} onKeyDown={handleKeyDown} onWheel={handleWheel} />
  );
}
...
function CommaNumberInput({ value, onChange, placeholder, style, max = 999999999.99 }) {
  const handleChange = (e) => {
...
export default function ManualStockInModal({ materials, vendors, onClose, onSave }) {
  ...
  const categoriesWithQty = useMemo(() => {
    const cats = selectedMaterials
      .filter(mat => (stockRowsByMaterial[mat.id] || []).some(r => (parseInt(r.qty) || 0) > 0))
  ...
  const generateMaterialRows = (material) => {
    ...
    return children.map(c => ({
      ...
      qty: '',
      damaged: '',
      unitCost: c.baseCost != null && c.baseCost > 0
        ? String(c.baseCost)
        : (material.baseCost != null && material.baseCost > 0 ? String(material.baseCost) : ''),
      minStockLevel: '10',
    }));
  };
  ...
  const toggleMaterial = (material) => {
    ...
    if (exists) {
      ...
      setStockRowsByMaterial(prev => { const n = { ...prev }; delete n[material.id]; return n; });
    } else {
  ...
  const totalDamaged = allRows.reduce((s, r) => s + (parseInt(r.damaged) || 0), 0);
  ...
  const totalInvoiceValue = useMemo(() => {
    return selectedMaterials.reduce((sum, mat) => {
      ...
      if (mode === 'total' && totalAmt) return sum + (parseFloat(totalAmt.replace(/,/g, '')) || 0);
      return sum + rows.reduce((s, r) => s + (parseInt(r.qty) || 0) * (parseFloat(r.unitCost) || 0), 0);
    }, 0);
  ...
  const step3Valid = () => {
    ...
    return selectedMaterials.every(mat => {
      ...
      const mode = materialCostModes[mat.id] || 'unit';
      const totalAmt = materialTotalAmounts[mat.id] || '';
      if (mode === 'total') return (parseFloat(totalAmt.replace(/,/g, '')) || 0) > 0;
  ...
  // ── Submit ────────────────────────────────────────────────────────────────
  const handleSubmit = () => {
    if (!step1Valid) { setInfoModal({ title: 'Validation Error', message: 'Select at least one material.' }); return; }
    if (!step2Valid()) { setInfoModal({ title: 'Validation Error', message: 'Enter received quantity for at least one item.' }); return; }
    if (!step3Valid()) { setInfoModal({ title: 'Validation Error', message: 'Complete all required invoice fields (reference number, delivery date, and cost).' }); return; }
    ...
    const siEntries = selectedMaterials.flatMap(mat => {
      ...
      const totalAmt = materialTotalAmounts[mat.id] || '';
      const totalQty = rows.reduce((s, r) => s + (parseInt(r.qty) || 0), 0);
      const computedUnitCost = mode === 'total' && totalAmt
    ...
    if (invoice.vendorId && categoriesWithQty.length > 0) {
      ...
      if (vendor) {
        ...
        const missingCats = categoriesWithQty.filter(cat => !vItems.includes(cat));
        if (missingCats.length > 0) {
          const updatedVendor = { ...vendor, itemsSupplied: [...new Set([...vItems, ...missingCats])] };
          const updatedVendors = vendors.map(v => v.id === vendor.id ? updatedVendor : v);
          if (typeof window !== 'undefined') {
            try { localStorage.setItem('pmp_vendors', JSON.stringify(updatedVendors)); } catch {}
          }
        }
      }
    }
  ...
  return (
    <div className="modal-overlay">
      ...
      <div className="modal-content" onClick={e => e.stopPropagation()}
        ...
        <div style={{ padding: '1.5rem 2rem', flexShrink: 0, borderBottom: '1px solid rgba(255,255,255,0.08)', background: '#131313' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem' }}>
            ...
            <button onClick={onClose}
              style={{ background: 'rgba(255,255,255,0.05)', border: 'none', borderRadius: '50%', width: '40px', height: '40px', cursor: 'pointer', color: 'var(--gray)', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s' }}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 0, position: 'relative', paddingLeft: '2rem', paddingRight: '2rem' }}>
            ...
            {[
              ...
              <React.Fragment key={i}>
                <div style={{ position: 'absolute', top: '14px', left, width: 'calc(33.33%)', height: '2px', background: 'rgba(255,255,255,0.08)', zIndex: 0, borderRadius: '2px', transform: 'translateX(-50%)' }} />
                <div style={{ position: 'absolute', top: '14px', left, width: fill ? 'calc(33.33%)' : '0%', height: '2px', background: 'linear-gradient(135deg, #FFDF9F 0%, #D4A843 100%)', zIndex: 1, borderRadius: '2px', transition: 'width 0.4s cubic-bezier(0.4,0,0.2,1)', boxShadow: fill ? '0 0 12px rgba(212,168,67,0.4)' : 'none', transform: 'translateX(-50%)' }} />
              </React.Fragment>
            ))}
            ...
            <div style={{ border: '1px solid var(--border)', borderRadius: '10px', overflow: 'hidden' }}>
              {groupedByCategory.length === 0 ? (
                ...
                <div key={group.category}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.875rem 1rem', background: 'rgba(255,255,255,0.04)', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                        ...
                        <div style={{ flex: 1 }}>
                          ...
                          {m.hasVariants && (
                            <div style={{ fontSize: '0.65rem', color: 'var(--gray)', marginTop: '0.1rem' }}>
                              {materials.filter(c => c.parentId === m.id).length} variants
                            </div>
                          )}
                        </div>
                        {isSelected && (
        ...
        {/* ── STEP 2: Stock Entry ──────────────────────────────────────────── */}
        {step === 2 && (
          <div className="si-wizard-grid" style={{ flex: 1, display: 'grid', gridTemplateColumns: '1fr 420px', minHeight: 0, borderTop: '1px solid rgba(255,255,255,0.08)' }}>

            {/* LEFT — Stock Entry */}
            <div className="si-wizard-panel" style={{ padding: '1.5rem 2rem', overflowY: 'auto', background: '#0E0E0E' }}>
              ...
              {selectedMaterials.map((mat, mIdx) => {
                ...
                return (
                  <div key={mat.id} style={{ marginBottom: mIdx < selectedMaterials.length - 1 ? '2rem' : '1.25rem' }}>
                    {/* Material header */}
                      ...
                      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                        <thead>
                          <tr style={{ background: 'rgba(0,0,0,0.2)', borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
                            <th style={{ padding: '0.875rem 1rem', textAlign: 'left', fontSize: '0.6rem', fontWeight: 700, color: 'var(--gray)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Item</th>
                            <th style={{ padding: '0.875rem 0.75rem', textAlign: 'center', fontSize: '0.6rem', fontWeight: 700, color: 'var(--gray)', textTransform: 'uppercase', letterSpacing: '0.08em', width: '110px' }}>Min Stock Level</th>
                            <th style={{ padding: '0.875rem 0.75rem', textAlign: 'center', fontSize: '0.6rem', fontWeight: 700, color: 'var(--gray)', textTransform: 'uppercase', letterSpacing: '0.08em', width: '130px' }}>Qty Received</th>
                            <th style={{ padding: '0.875rem 0.75rem', textAlign: 'center', fontSize: '0.6rem', fontWeight: 700, color: 'var(--gray)', textTransform: 'uppercase', letterSpacing: '0.08em', width: '110px' }}>Damaged</th>
                          </tr>
                        </thead>
                        <tbody>
                          {rows.map((row, idx) => {
                            ...
                            return (
                              <tr key={row.materialId} style={{ background: idx % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.015)', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                                ...
                                <td style={{ padding: '1rem 0.75rem', textAlign: 'center' }}>
                                  <IntegerInput value={row.minStockLevel}
                                    onChange={val => updateStockRow(mat.id, idx, 'minStockLevel', val)}
                                    min={1} max={9999} placeholder="10"
                                    style={{ textAlign: 'center', width: '80px', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', color: '#E5E2E1', fontWeight: 600, padding: '0.5rem', fontSize: '0.85rem' }} />
                                </td>
                                <td style={{ padding: '1rem 0.75rem', textAlign: 'center' }}>
                                  <IntegerInput value={row.qty}
                                    onChange={val => updateStockRow(mat.id, idx, 'qty', val)}
                                    min={0} max={99999} placeholder="0"
                                    style={{ textAlign: 'center', width: '80px', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', color: q > 0 ? '#D4A843' : 'var(--gray)', fontWeight: 700, padding: '0.5rem', fontSize: '0.9rem' }} />
                                </td>
                                <td style={{ padding: '1rem 0.75rem', textAlign: 'center' }}>
                                  <IntegerInput value={row.damaged}
                                    onChange={val => updateStockRow(mat.id, idx, 'damaged', val)}
                                    min={0} max={99999} placeholder="0"
                                    qtyValue={row.qty}
                                    style={{ textAlign: 'center', width: '80px', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', color: d > 0 ? '#F87171' : 'var(--gray)', fontWeight: 600, padding: '0.5rem', fontSize: '0.9rem' }} />
                                </td>
                              </tr>
                            );
                          })}
            ...
            <div className="si-wizard-panel" style={{ padding: '1.5rem 2rem', background: '#131313', borderLeft: '1px solid rgba(255,255,255,0.08)', overflowY: 'auto' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.25rem' }}>
                <div style={{ width: '36px', height: '36px', borderRadius: '10px', background: 'rgba(212,168,67,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#D4A843" strokeWidth="2">
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', opacity: 0.5, pointerEvents: 'none' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
                  <div>
                    <label style={{ display: 'block', fontSize: '0.65rem', fontWeight: 700, color: 'var(--gray)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '0.5rem' }}>Reference / OR Number</label>
              ...
              <div style={{ marginTop: '2rem', padding: '1rem', background: 'rgba(212,168,67,0.08)', border: '1px solid rgba(212,168,67,0.2)', borderRadius: '8px', textAlign: 'center' }}>
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#D4A843" strokeWidth="2" style={{ margin: '0 auto 0.5rem', display: 'block' }}>
          ...
          <div className="si-wizard-grid" style={{ flex: 1, display: 'grid', gridTemplateColumns: '1fr 420px', minHeight: 0, borderTop: '1px solid rgba(255,255,255,0.08)' }}>
            ...
            <div className="si-wizard-panel" style={{ padding: '1.5rem 2rem', overflowY: 'auto', background: '#0E0E0E' }}>
              ...
              {selectedMaterials.map((mat, mIdx) => {
                ...
                const matQty = rows.reduce((s, r) => s + (parseInt(r.qty) || 0), 0);
                const matDmg = rows.reduce((s, r) => s + (parseInt(r.damaged) || 0), 0);
                const matGood = matQty - matDmg;

                return (
                  <div key={mat.id} style={{ marginBottom: mIdx < selectedMaterials.length - 1 ? '1.5rem' : 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.75rem' }}>
                      ...
                      <div>
                        ...
                        <div style={{ fontSize: '0.6rem', color: 'var(--gray)' }}>{rows.length} variant{rows.length !== 1 ? 's' : ''} with qty</div>
                      </div>
                    </div>
                      ...
                      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.8rem' }}>
                        <thead>
                          <tr style={{ background: 'rgba(0,0,0,0.2)', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                            ...
                            <th style={{ padding: '0.625rem 0.5rem', textAlign: 'center', fontSize: '0.6rem', fontWeight: 700, color: 'var(--gray)', textTransform: 'uppercase', width: '60px' }}>Qty</th>
                            <th style={{ padding: '0.625rem 0.5rem', textAlign: 'center', fontSize: '0.6rem', fontWeight: 700, color: 'var(--gray)', textTransform: 'uppercase', width: '60px' }}>Dmg</th>
                          </tr>
                        </thead>
                        <tbody>
                          {rows.map((row, idx) => {
                            ...
                            const d = parseInt(row.damaged) || 0;
                            return (
                              <tr key={row.materialId} style={{ borderBottom: idx < rows.length - 1 ? '1px solid rgba(255,255,255,0.03)' : 'none' }}>
                                <td style={{ padding: '0.625rem 0.75rem' }}>
                                  <div style={{ fontWeight: 600, color: '#E5E2E1', fontSize: '0.75rem' }}>{row.variantLabel}</div>
                    ...
                    {selectedMaterials.filter(m => (stockRowsByMaterial[m.id] || []).some(r => (parseInt(r.qty) || 0) > 0)).length > 1 && (
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem' }}>
                        <div style={{ padding: '0.5rem', background: 'rgba(255,255,255,0.03)', borderRadius: '6px', textAlign: 'center' }}>
                          ...
                          <div style={{ fontSize: '0.9rem', fontWeight: 700, color: '#E5E2E1' }}>{matGood} <span style={{ fontSize: '0.6rem' }}>pcs</span></div>
                        </div>
                        <div style={{ padding: '0.5rem', background: 'rgba(248,113,113,0.06)', borderRadius: '6px', textAlign: 'center' }}>
                          ...
                          <div style={{ fontSize: '0.9rem', fontWeight: 700, color: '#F87171' }}>{matDmg} <span style={{ fontSize: '0.6rem' }}>pcs</span></div>
                        </div>
                      </div>
                    )}
                  </div>
            ...
            <div className="si-wizard-panel" style={{ padding: '1.5rem 2rem', background: '#131313', borderLeft: '1px solid rgba(255,255,255,0.08)', overflowY: 'auto' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                ...
                <div>
                  <label style={{ fontSize: '0.65rem', fontWeight: 700, color: 'var(--gray)', textTransform: 'uppercase', letterSpacing: '0.08em', display: 'block', marginBottom: '0.5rem' }}>Supplier</label>
                  <SupplierCombobox
                    value={invoice.vendorId}
                    vendorName={invoice.vendorName}
                    onChange={(id, name) => setInvoice(p => ({ ...p, vendorId: id, vendorName: name }))}
                    vendors={vendors}
                  ...
                  {categoriesWithQty.length > 0 && (
                    <div style={{ fontSize: '0.62rem', color: 'var(--gray)', marginTop: '0.4rem', display: 'flex', alignItems: 'center', gap: '0.35rem' }}>
                      ...
                      Showing suppliers for: <span style={{ color: '#D4A843', fontWeight: 600 }}>{categoriesWithQty.join(', ')}</span>
                    </div>
                  )}
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
                  <div>
                    <label style={{ fontSize: '0.65rem', fontWeight: 700, color: 'var(--gray)', textTransform: 'uppercase', letterSpacing: '0.08em', display: 'block', marginBottom: '0.5rem' }}>Reference / OR Number <span style={{ color: '#ef4444' }}>*</span></label>
                    <input type="text" value={invoice.referenceNo}
                      onChange={e => setInvoice(p => ({ ...p, referenceNo: e.target.value.slice(0, 50) }))}
                      placeholder="INV-2024-001"
                      style={{ width: '100%', padding: '0.625rem 0.75rem', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', color: '#E5E2E1', fontSize: '0.85rem', outline: 'none', boxSizing: 'border-box' }} />
                  </div>
                  <div>
                    <label style={{ fontSize: '0.65rem', fontWeight: 700, color: 'var(--gray)', textTransform: 'uppercase', letterSpacing: '0.08em', display: 'block', marginBottom: '0.5rem' }}>Delivery Date <span style={{ color: '#ef4444' }}>*</span></label>
                    <input type="date" value={invoice.deliveryDate}
                      onChange={e => setInvoice(p => ({ ...p, deliveryDate: e.target.value }))}
                      max={new Date().toISOString().split('T')[0]}
                      style={{ width: '100%', padding: '0.625rem 0.75rem', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', color: '#E5E2E1', fontSize: '0.85rem', outline: 'none', colorScheme: 'dark', boxSizing: 'border-box' }} />
                  </div>
                </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                  {selectedMaterials.map(mat => {
                    ...
                    const totalAmt = materialTotalAmounts[mat.id] || '';
                    const totalQty = rows.reduce((s, r) => s + (parseInt(r.qty) || 0), 0);
                    const computedUnitCost = totalQty > 0 && totalAmt
                            ...
                            <div style={{ display: 'flex', background: 'rgba(0,0,0,0.3)', borderRadius: '6px', padding: '2px' }}>
                              {[['unit', 'Unit'], ['total', 'Total']].map(([val, label]) => (
                                <button key={val} type="button"
                                  onClick={() => setMaterialCostModes(p => ({ ...p, [mat.id]: val }))}
                                  style={{
                                    padding: '0.25rem 0.5rem', fontSize: '0.6rem', fontWeight: 700, borderRadius: '4px', border: 'none', cursor: 'pointer',
                                    background: mode === val ? '#D4A843' : 'transparent',
                                    color: mode === val ? '#000' : 'var(--gray)',
                          ...
                          {mode === 'total' ? (
                            ...
                            <div style={{ padding: '0.75rem', background: 'rgba(212,168,67,0.05)', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                              ...
                              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                ...
                                <CommaNumberInput value={totalAmt}
                                  onChange={val => setMaterialTotalAmounts(p => ({ ...p, [mat.id]: val }))}
                                  placeholder="0.00"
                              ...
                              {computedUnitCost > 0 && (
                                <div style={{ fontSize: '0.65rem', color: 'var(--gray)', marginTop: '0.5rem', paddingTop: '0.5rem', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
                                  Unit Cost: <strong style={{ color: '#D4A843' }}>{formatPrice(computedUnitCost)}</strong>
                                </div>
                              )}
                            </div>
                          ) : (
                            ...
                            <>
                              ...
                              {rows.length > 1 && (
                                <div style={{ display: 'flex', gap: '0.5rem', padding: '0.75rem', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                                  <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: '0.5rem', background: 'rgba(255,255,255,0.06)', padding: '0.4rem 0.6rem', borderRadius: '6px' }}>
                                    ...
                                    <DecimalInput value={applyAllCost} onChange={val => setApplyAllCost(val)}
                                      placeholder="0.00"
                                      style={{ flex: 1, background: 'none', border: 'none', color: 'var(--white)', fontSize: '0.8rem', outline: 'none' }} />
                                  </div>
                                  <button type="button"
                                    onClick={() => {
                                      const allMatRows = stockRowsByMaterial[mat.id] || [];
                                      const updated = allMatRows.map(r => ({ ...r, unitCost: applyAllCost }));
                                      setStockRowsByMaterial(p => ({ ...p, [mat.id]: updated }));
                                    }}
                              ...
                              {rows.map((row, idx) => {
                                ...
                                const allMatRows = stockRowsByMaterial[mat.id] || [];
                                const origIdx = allMatRows.findIndex(r => r.materialId === row.materialId);

                                return (
                                  <div key={row.materialId}
                                    style={{ display: 'grid', gridTemplateColumns: '1fr 80px 80px', padding: '0.625rem 0.75rem', borderTop: '1px solid rgba(255,255,255,0.04)', alignItems: 'center' }}>
                                    <span style={{ fontSize: '0.8rem', color: 'var(--white)', fontWeight: 500 }}>{row.variantLabel}</span>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.25rem', background: 'rgba(255,255,255,0.06)', padding: '0.35rem 0.5rem', borderRadius: '6px', justifyContent: 'center' }}>
                                      ...
                                      <DecimalInput value={row.unitCost}
                                        onChange={val => { if (origIdx >= 0) updateStockRow(mat.id, origIdx, 'unitCost', val); }}
                                        placeholder="0.00"
                ...
                {/* Notes */}
                <div>
                  <label style={{ fontSize: '0.65rem', fontWeight: 700, color: 'var(--gray)', textTransform: 'uppercase', letterSpacing: '0.08em', display: 'block', marginBottom: '0.5rem' }}>Notes <span style={{ fontWeight: 400 }}>(Optional)</span></label>
                  <textarea value={invoice.notes}
                    onChange={e => setInvoice(p => ({ ...p, notes: e.target.value.slice(0, 500) }))}
                    placeholder="Optional notes..."
                    style={{ width: '100%', padding: '0.65rem 0.875rem', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', color: '#E5E2E1', fontSize: '0.8rem', outline: 'none', resize: 'vertical', minHeight: '50px', boxSizing: 'border-box' }} />
                </div>
                ...
                <div>
                  <label style={{ display: 'block', fontSize: '0.65rem', fontWeight: 700, color: 'var(--gray)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '0.5rem' }}>
                    Proof of Purchase / Receipt <span style={{ fontWeight: 400 }}>(Optional)</span>
                  </label>
                  <label
                    style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', padding: '1.25rem', border: '2px dashed rgba(255,255,255,0.15)', borderRadius: '10px', background: 'rgba(0,0,0,0.15)', cursor: 'pointer', transition: 'all 0.2s' }}
                ...
                {totalInvoiceValue > 0 && (
                  <div style={{ padding: '1rem', background: 'rgba(212,168,67,0.08)', border: '1px solid rgba(212,168,67,0.25)', borderRadius: '10px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.75rem' }}>
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#D4A843" strokeWidth="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
                    ...
                    {selectedMaterials.length > 1 && (
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', marginBottom: '0.75rem' }}>
                        {selectedMaterials.map(mat => {
                          ...
                          if (mode === 'total' && totalAmt) {
                            subtotal = parseFloat(totalAmt.replace(/,/g, '')) || 0;
                            displayUnit = totalQty > 0 ? subtotal / totalQty : 0;
                          } else {
                            subtotal = rows.reduce((s, r) => s + (parseInt(r.qty) || 0) * (parseFloat(r.unitCost) || 0), 0);
                            const costs = rows.map(r => parseFloat(r.unitCost) || 0);
                            displayUnit = costs.every(c => c === costs[0]) ? costs[0] : null;
                          }
                          return (
                            <div key={mat.id} style={{ padding: '0.75rem', background: 'rgba(0,0,0,0.2)', borderRadius: '8px' }}>
                              <div style={{ fontSize: '0.7rem', color: '#E5E2E1', fontWeight: 600, marginBottom: '0.5rem' }}>{mat.name}</div>
                              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '0.5rem' }}>
                                <div>
                                  <div style={{ fontSize: '0.55rem', color: 'var(--gray)', textTransform: 'uppercase' }}>Total Qty</div>
                                  <div style={{ color: '#D4A843', fontWeight: 700, fontSize: '0.75rem' }}>{totalQty} <span style={{ fontWeight: 400, color: 'var(--gray)' }}>pcs</span></div>
                                </div>
                                <div>
                                  <div style={{ fontSize: '0.55rem', color: 'var(--gray)', textTransform: 'uppercase' }}>Unit Cost</div>
                                  <div style={{ color: '#D4A843', fontWeight: 700, fontSize: '0.75rem' }}>
                                    {displayUnit !== null ? formatPrice(displayUnit) : <span style={{ fontSize: '0.6rem', color: 'var(--gray)' }}>Varies</span>}
                                  </div>
                                </div>
                                <div>
                                  <div style={{ fontSize: '0.55rem', color: 'var(--gray)', textTransform: 'uppercase' }}>Subtotal</div>
                                  <div style={{ color: '#facc15', fontWeight: 700, fontSize: '0.75rem' }}>{formatPrice(subtotal)}</div>
                                </div>
                              </div>
                            </div>
                          );
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.75rem 0', borderTop: selectedMaterials.length > 1 ? '1px solid rgba(212,168,67,0.3)' : 'none' }}>
            ...
            {step === 3 && totalInvoiceValue > 0 && (
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', fontSize: '0.75rem' }}>
                <span style={{ color: 'var(--gray)' }}>
                  Effective: <strong style={{ color: '#FACC15' }}>{formatPrice(effectiveValue)}</strong>
                </span>
                <span style={{ color: 'var(--gray)' }}>
                  Receipt Total: <strong style={{ color: '#FACC15' }}>{formatPrice(totalInvoiceValue)}</strong>
                </span>
              </div>
            )}
      ...
      <AddVendorQuickModal
        ...
        onAdd={(data) => {
          const newVendor = { ...data, id: crypto.randomUUID(), createdAt: new Date().toISOString() };
          // Persist to localStorage so parent can pick it up
          if (typeof window !== 'undefined') {
            try {
              const existing = JSON.parse(localStorage.getItem('pmp_vendors') || '[]');
              localStorage.setItem('pmp_vendors', JSON.stringify([...existing, newVendor]));
            } catch {}
          }
          setInvoice(p => ({ ...p, vendorId: newVendor.id, vendorName: newVendor.name }));
