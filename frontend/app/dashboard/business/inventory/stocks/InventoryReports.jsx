...
// ── Shared Styles ──────────────────────────────────────────────────────────────
const thStyle = {
  padding: '0.5rem 0.75rem', textAlign: 'left', color: '#374151',
...
export default function InventoryReports({ materials, stockOuts }) {
  ...
  const damageReport = useMemo(() => {
    ...
    const grouped = {};
    damages.forEach(so => {
      const cat = materials?.find(m => m.id === so.materialId)?.category || so.category || 'Uncategorized';
      if (!grouped[cat]) grouped[cat] = [];
      grouped[cat].push(so);
    });
    const totalQty = damages.reduce((s, so) => s + (so.quantity || 0), 0);
  ...
  const stockOutReport = useMemo(() => {
    ...
    const totalQty = filteredStockOuts.reduce((s, so) => s + (so.quantity || 0), 0);
    const totalLoss = filteredStockOuts.reduce((s, so) => s + (so.totalLoss || 0), 0);
    return { grouped, totalQty, totalLoss, count: filteredStockOuts.length };
  }, [filteredStockOuts, materials]);

  // Stock Summary Data
  const stockSummary = useMemo(() => {
    const items = (materials || []).filter(m => !m.parentId).map(m => {
      ...
      if (hasChildren) {
        ...
        avgCost = allChildrenBatches.length > 0 ? allChildrenBatches.reduce((s, b) => s + (b.unitCost || 0), 0) / allChildrenBatches.length : (m.baseCost || 0);
      } else {
        ...
        totalStock = goodStock + damaged;
        avgCost = batches.length > 0 ? batches.reduce((s, b) => s + (b.unitCost || 0), 0) / batches.length : (m.baseCost || 0);
      }
    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: '0.75rem', marginBottom: '1.5rem', alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        <button onClick={handlePrint}
          style={{ padding: '0.6rem 1.25rem', background: 'var(--gold)', border: 'none', borderRadius: '8px', color: '#000', fontWeight: 700, cursor: 'pointer', fontSize: '0.85rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
      ...
      <div style={{ border: '1px solid var(--border)', borderRadius: '12px', overflow: 'hidden', background: 'var(--dark)' }}>
        ...
        <div style={{ padding: '1.5rem 2rem', borderBottom: '1px solid var(--border)' }}>
          ...
          <p style={{ margin: '0.25rem 0 0 0', fontSize: '0.8rem', color: 'var(--gray)' }}>Date Range: {dateLabel} | Generated: {new Date().toLocaleDateString('en-PH', { year: 'numeric', month: 'long', day: 'numeric' })}</p>
        </div>
              ...
              <div className="summary-card" style={{ background: 'rgba(212,168,67,0.08)', borderColor: 'rgba(212,168,67,0.3)' }}>
                <div className="summary-content">
                  <span className="summary-value" style={{ color: '#D4A843' }}>
                    ₱{damageReport.totalLoss.toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </span>
                ...
                <tbody>
                  {Object.entries(damageReport.grouped).map(([cat, records]) => (
                    <React.Fragment key={cat}>
                      <tr><td colSpan={7} style={{ padding: '0.5rem 0.75rem', background: 'rgba(212,168,67,0.06)', fontWeight: 700, color: '#D4A843', fontSize: '0.75rem', textTransform: 'uppercase' }}>{cat}</td></tr>
                      {records.map((so, idx) => (
          ...
          <div style={{ padding: '1.5rem 2rem' }}>
            <div className="inventory-summary" style={{ marginBottom: '1.5rem' }}>
              ...
              <div className="summary-card" style={{ background: 'rgba(212,168,67,0.08)', borderColor: 'rgba(212,168,67,0.3)' }}>
                <div className="summary-content">
                  <span className="summary-value" style={{ color: '#D4A843' }}>
                    ₱{stockOutReport.totalLoss.toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </span>
                  <span className="summary-label">Total Loss Value</span>
                </div>
              </div>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.8rem' }}>
                ...
                <tbody>
                  {Object.entries(stockOutReport.grouped).map(([cat, records]) => (
                    <React.Fragment key={cat}>
                      <tr><td colSpan={6} style={{ padding: '0.5rem 0.75rem', background: 'rgba(212,168,67,0.06)', fontWeight: 700, color: '#D4A843', fontSize: '0.75rem', textTransform: 'uppercase' }}>{cat}</td></tr>
                      {records.map((so, idx) => (
                        <tr key={so.id} style={{ background: idx % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.01)', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                          <td style={{ padding: '0.5rem 0.75rem', color: 'var(--gray)' }}>{new Date(so.dateIssued || so.createdAt).toLocaleDateString('en-PH')}</td>
                          <td style={{ padding: '0.5rem 0.75rem', fontWeight: 600, color: '#E5E2E1' }}>{so.materialName}</td>
                          <td style={{ padding: '0.5rem 0.75rem', textAlign: 'center' }}>
                            <span style={{ fontSize: '0.65rem', fontWeight: 700, padding: '0.15rem 0.5rem', borderRadius: '4px', background: 'rgba(255,255,255,0.1)', color: '#E5E2E1' }}>{so.issueType}</span>
                          </td>
                          <td style={{ padding: '0.5rem 0.75rem', textAlign: 'center', fontWeight: 700, color: '#ef4444' }}>-{so.quantity} {so.uom || 'pcs'}</td>
                          <td style={{ padding: '0.5rem 0.75rem', textAlign: 'right', fontWeight: 700, color: '#ef4444', fontFamily: 'monospace' }}>₱{(so.totalLoss || 0).toFixed(2)}</td>
                          <td style={{ padding: '0.5rem 0.75rem', textAlign: 'center', color: '#E5E2E1', fontSize: '0.75rem' }}>{so.performedBy || 'Unknown'}</td>
          ...
          <div style={{ padding: '1.5rem 2rem' }}>
            <div className="inventory-summary" style={{ marginBottom: '1.5rem', gridTemplateColumns: 'repeat(4, 1fr)' }}>
              ...
              <div className="summary-card" style={{ background: 'rgba(212,168,67,0.08)', borderColor: 'rgba(212,168,67,0.3)' }}>
                <div className="summary-content">
                  <span className="summary-value" style={{ color: '#D4A843' }}>
                    ₱{stockSummary.totalValue.toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </span>
              ...
              <div className="summary-card summary-card-danger">
                <div className="summary-content">
                  <span className="summary-value" style={{ color: '#ef4444' }}>
                    ₱{stockSummary.totalDamageValue.toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </span>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.8rem' }}>
              ...
              <tbody>
                {stockSummary.items.map((item, idx) => (
                  <tr key={item.id} style={{ background: idx % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.01)', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                    ...
                    <td style={{ padding: '0.5rem 0.75rem', textAlign: 'center', fontWeight: 700, color: item.damaged > 0 ? '#ef4444' : '#6b7280' }}>{item.damaged}</td>
