...
export default function BOMVerification({ order, onClose }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2000, backdropFilter: 'blur(4px)' }} onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} style={{ background: 'var(--dark)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '16px', width: '90%', maxWidth: '520px', overflow: 'hidden', boxShadow: '0 24px 48px rgba(0,0,0,0.5)' }}>
        {/* Header */}
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1.25rem 1.5rem', borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
          <div>
            <div style={{ fontSize: '0.6rem', color: 'var(--green)', textTransform: 'uppercase', letterSpacing: '0.15em', fontWeight: 700, marginBottom: '0.25rem' }}>Bill of Materials</div>
            <h2 style={{ margin: 0, fontSize: '1.15rem', fontWeight: 700, color: 'var(--white)' }}>BOM Verification</h2>
          </div>
        ...
        <div style={{ padding: '1.5rem' }}>
          ...
          <div style={{ padding: '1rem', background: order.bom.verified ? 'rgba(34,197,94,0.08)' : 'rgba(239,68,68,0.08)', borderRadius: '10px', border: `1px solid ${order.bom.verified ? 'rgba(34,197,94,0.2)' : 'rgba(239,68,68,0.2)'}`, marginBottom: '1.25rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
              <div style={{ width: '40px', height: '40px', borderRadius: '50%', background: order.bom.verified ? 'rgba(34,197,94,0.15)' : 'rgba(239,68,68,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={order.bom.verified ? 'var(--green)' : 'var(--red)'} strokeWidth="2">
              ...
              <div>
                <div style={{ fontSize: '0.9rem', fontWeight: 700, color: order.bom.verified ? 'var(--green)' : 'var(--red)' }}>
          ...
          {order.bom.items && order.bom.items.length > 0 && (
            <div>
              <h4 style={{ margin: '0 0 0.75rem 0', fontSize: '0.7rem', fontWeight: 700, color: 'var(--gray)', textTransform: 'uppercase' }}>Material Requirements</h4>
              <div style={{ border: '1px solid rgba(255,255,255,0.08)', borderRadius: '8px', overflow: 'hidden' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.8rem' }}>
                  <thead>
                    <tr style={{ background: 'rgba(0,0,0,0.3)', borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
                      <th style={{ padding: '0.6rem 0.75rem', textAlign: 'left', color: 'var(--gray)', fontWeight: 700, fontSize: '0.65rem', textTransform: 'uppercase' }}>Material</th>
                      <th style={{ padding: '0.6rem 0.75rem', textAlign: 'center', color: 'var(--gray)', fontWeight: 700, fontSize: '0.65rem', textTransform: 'uppercase' }}>Required</th>
                      <th style={{ padding: '0.6rem 0.75rem', textAlign: 'center', color: 'var(--gray)', fontWeight: 700, fontSize: '0.65rem', textTransform: 'uppercase' }}>Available</th>
                      <th style={{ padding: '0.6rem 0.75rem', textAlign: 'center', color: 'var(--gray)', fontWeight: 700, fontSize: '0.65rem', textTransform: 'uppercase' }}>Status</th>
                    </tr>
                  </thead>
                  <tbody>
                    {order.bom.items.map((item, idx) => {
                      ...
                      return (
                        <tr key={idx} style={{ borderBottom: idx < order.bom.items.length - 1 ? '1px solid rgba(255,255,255,0.04)' : 'none' }}>
                          <td style={{ padding: '0.6rem 0.75rem', color: 'var(--white)', fontWeight: 600 }}>{item.name}</td>
                          <td style={{ padding: '0.6rem 0.75rem', textAlign: 'center', color: 'var(--white)', fontFamily: 'monospace' }}>{item.required}</td>
                          <td style={{ padding: '0.6rem 0.75rem', textAlign: 'center', color: ok ? 'var(--green)' : 'var(--red)', fontFamily: 'monospace', fontWeight: 700 }}>{item.available}</td>
                          <td style={{ padding: '0.6rem 0.75rem', textAlign: 'center' }}>
                            <span style={{ fontSize: '0.65rem', fontWeight: 700, padding: '0.15rem 0.5rem', borderRadius: '4px', background: ok ? 'rgba(34,197,94,0.15)' : 'rgba(239,68,68,0.15)', color: ok ? 'var(--green)' : 'var(--red)' }}>
                              {ok ? 'OK' : 'SHORT'}
                            </span>
                          </td>
                        </tr>
          ...
          {(!order.bom.items || order.bom.items.length === 0) && (
            <div style={{ textAlign: 'center', padding: '1.5rem', color: 'var(--gray)', fontSize: '0.8rem' }}>
