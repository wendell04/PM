    ...
    <div style={{ minHeight: '100vh', background: '#f8f9fb' }}>
      <div style={{ maxWidth: 1040, margin: '0 auto', padding: '24px 16px 48px' }}>
        ...
        <div style={{ ...CARD, background: '#fffbeb', borderColor: '#fcd34d', marginBottom: 16 }}>
          ...
          <p style={{ margin: '4px 0 0', fontSize: '.78rem', color: '#6b7280', lineHeight: 1.5 }}>
        ...
        <p style={{ color: '#6b7280', fontSize: '.85rem', margin: '0 0 18px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) minmax(0,340px)', gap: 16, alignItems: 'start' }}>
              ...
              {addresses.length === 0 ? (
                ...
                <>
                  <select
                    ...
                    style={{ width: '100%', padding: '9px 10px', borderRadius: 8, border: '1px solid #e5e7eb',
                      background: '#fff', fontSize: '.85rem', color: '#111827' }}
                  >
                    {addresses.map(a => (
                  ...
                  {addr && (
                    <p style={{ margin: '8px 0 0', fontSize: '.78rem', color: '#6b7280', lineHeight: 1.5 }}>
                  ...
                  {[
                    ...
                    <button key={opt.id} type="button" onClick={() => setSchedule(opt.id)}
                      style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10,
                        ...
                        border: `1.5px solid ${schedule === opt.id ? '#111827' : '#e5e7eb'}`, background: '#fff' }}>
                      <span>
                        <span style={{ display: 'block', fontWeight: 700, fontSize: '.85rem' }}>{opt.title}</span>
                        <span style={{ fontSize: '.74rem', color: '#6b7280' }}>{opt.sub}</span>
                      </span>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {available.map(m => (
                    <button key={m.id} type="button" onClick={() => setMethod(m.id)}
                      style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10,
                        textAlign: 'left', padding: '11px 12px', borderRadius: 10, cursor: 'pointer',
                        border: `1.5px solid ${method === m.id ? '#111827' : '#e5e7eb'}`, background: '#fff' }}>
                      <span>
                        <span style={{ display: 'block', fontWeight: 700, fontSize: '.85rem' }}>{m.label}</span>
                        <span style={{ fontSize: '.74rem', color: '#6b7280' }}>{m.sub}</span>
                      </span>
                      <span style={{ width: 16, height: 16, borderRadius: '50%', flexShrink: 0,
                        border: `2px solid ${method === m.id ? '#111827' : '#d1d5db'}`,
                        background: method === m.id ? '#111827' : 'transparent' }} />
                    </button>
                  ))}
                </div>
              )}
              {hasMadeToOrder && payEnabled.cod !== false && (
                <p style={{ margin: '10px 0 0', fontSize: '.72rem', color: '#6b7280' }}>
          ...
          <div style={{ ...CARD, position: 'sticky', top: 80 }}>
            ...
            {items.length === 0 ? (
              <p style={{ margin: 0, fontSize: '.82rem', color: '#6b7280' }}>Your cart is empty.</p>
            ) : (
                  ...
                  <div key={it.lineId ?? i} style={{ display: 'flex', gap: 10 }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: '.82rem', fontWeight: 700, lineHeight: 1.3 }}>
                        {it.productName}
                        {it.variantName && <span style={{ fontWeight: 500, color: '#6b7280' }}> - {it.variantName}</span>}
                      </div>
            ...
            <div style={{ borderTop: '1px solid #e5e7eb', paddingTop: 10, display: 'flex', flexDirection: 'column', gap: 6, fontSize: '.82rem' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', borderTop: '1px solid #e5e7eb',
            ...
            <button type="button" disabled
              style={{ width: '100%', marginTop: 14, padding: '12px', borderRadius: 10, border: 'none',
                background: '#111827', color: '#fff', fontWeight: 800, fontSize: '.9rem', opacity: .55, cursor: 'not-allowed' }}>
              Pay {peso(payNow)}
            </button>
            <p style={{ margin: '8px 0 0', fontSize: '.7rem', color: '#9ca3af', textAlign: 'center' }}>
...
function Row({ label, value, hint }) {
  return (
    <div>
      ...
      {hint && <div style={{ fontSize: '.68rem', color: '#9ca3af', marginTop: 1 }}>{hint}</div>}
    </div>
  );
}
